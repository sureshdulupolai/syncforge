...
class SyncForge:
    ...
    def create_table(
        ...
        if self._dev_mode:
            ...
            print(f"\n[SyncForge Local] ✅ Successfully registered table '{table_name}'. (ID: {sync_id})")
            print(f"[SyncForge Local] 💡 Tip: You can verify this by calling `sf.listed_table()` in your code.\n")
            return res
        ...
        try:
            ...
            if not hasattr(self, "_live_tables_map"):
                self._live_tables_map = {}
            
            # Extract physical name returned by server
            physical_name = res.get("scoped_table_name") or res.get("sync_id") or res.get("table") or table_name
            self._live_tables_map[table_name] = physical_name
    ...
    def listed_table(self) -> dict:
        """
        Returns a standard SDK response mapping logical table names provided by the 
        client to the actual physical table names (or sync_ids) generated by SyncForge.
        """
        data = {"client_table": [], "created_table": []}
        if self._dev_mode:
            if hasattr(self, "_local_tables"):
                data["client_table"] = list(self._local_tables.keys())
                data["created_table"] = list(self._local_tables.values())
        else:
            if hasattr(self, "_live_tables_map"):
                data["client_table"] = list(self._live_tables_map.keys())
                data["created_table"] = list(self._live_tables_map.values())
                
        return {
            "success": True,
            "status": "ok",
            "message": "Mapped tables fetched successfully.",
            "data": data
        }

    def latest_table(self) -> dict:
        """
        Returns the most recently registered table mapping.
        """
        mapping = self.listed_table().get("data", {})
        if mapping.get("client_table"):
            return {
                "logical": mapping["client_table"][-1],
                "physical": mapping["created_table"][-1]
            }
        return {}
    ...
    def filter_table(self, names: Union[str, List[str]]) -> Union[bool, Dict[str, bool]]:
        ...
        Automatically cross-references both physical generated names AND your logical names.
        ...
        """
        mapping = self.listed_table().get("data", {})
        all_logical = set(mapping.get("client_table", []))
        all_physical = set(self.all_table())
        all_tables = all_physical.union(all_logical)
