...
class SyncForge:
    ...
    def create_table(
        ...
        if self._dev_mode:
            ...
            print(f"[SyncForge Local] 💡 Tip: You can verify this by calling `sf.table_mapping()` in your code.\n")
            return res
    ...
    def table_mapping(self) -> dict:
        """
        Returns a mapping of logical table names provided by the client to the 
        actual physical table names (or sync_ids) generated by SyncForge.
        
        Returns:
            Dict[str, List[str]]: {"client_table": [...], "created_table": [...]}
        """
        if self._dev_mode:
            if hasattr(self, "_local_tables"):
                return {
                    "client_table": list(self._local_tables.keys()),
                    "created_table": list(self._local_tables.values())
                }
        else:
            if hasattr(self, "_live_tables_map"):
                return {
                    "client_table": list(self._live_tables_map.keys()),
                    "created_table": list(self._live_tables_map.values())
                }
        return {"client_table": [], "created_table": []}
